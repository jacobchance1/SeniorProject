...
void normalization_and_padding(hls::stream_of_blocks<raw> &image,
                                fixed_t (&padded_image)[(28 + (5 - 1))][(28 + (5 - 1))]);
# 2 "utils.cc" 2
                                ...
                                fixed_t (&padded_image)[(28 + (5 - 1))][(28 + (5 - 1))])
{
 hls::read_lock<raw> img_lock(image);
    ...
    VITIS_LOOP_8_1: for (int i =0; i< 28; ++i){
        VITIS_LOOP_9_2: for (int j=0;j< 28; ++j){
            padded_image[i+(5 - 1)/2][j+(5 - 1)/2] = img_lock[i][j]/((fixed_t)255.0);
